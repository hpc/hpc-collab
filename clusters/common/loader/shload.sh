...
CLUSTERNAME=${CLUSTERNAME:-${HOSTNAME:0:2}}
if [ -d "/${CLUSTERNAME}" ] ; then
  ANCHOR=${ANCHOR:-/${CLUSTERNAME}/common/provision}
else
  ANCHOR=${ANCHOR:-/vagrant/common/provision}
fi
...
declare -x PROVISION_SRC_BIN_D=${PROVISION_SRC_D}/bin
