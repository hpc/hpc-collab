...
IsLANL() {
  ...
  ping -c 1 -i 1 -n ${IS_LANL_PINGABLE} >/dev/null 2>&1
  rc=$?
...
SetFlags() {
  ...
  if [ ! -d ${VC} ] ; then
    HALT_ERREXIT="true"
    ErrExit ${EX_OSFILE} "VC: ${VC} not mounted? -- ensure that \", disabled: true\" is removed from the synced_folder entry in Vagrantfile for ${HOSTNAME}"
  fi
